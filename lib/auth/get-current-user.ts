...
export async function getCurrentUserId() {
  ...
  if (process.env.ENABLE_AUTH === 'false') {
    // Guard: Prevent disabling auth in Tavus Cloud deployments
    if (process.env.TAVUS_CLOUD_DEPLOYMENT === 'true') {
      throw new Error(
        'ENABLE_AUTH=false is not allowed in TAVUS_CLOUD_DEPLOYMENT'
      )
    }
